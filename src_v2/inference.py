...
"""
Full inference pipeline for Vesuvius Challenge Surface Detection — Pipeline V2.

Uses the UNet3DDeepSup model which returns only main logits during eval,
so sliding window inference is identical to V1 mechanically.

Pipeline:
  1. Load pre-trained UNet3DDeepSup weights
  2. Load test volume
  3. Sliding window with GPU feature extraction per-patch
  4. Test-time augmentation (8 flips)
  5. Argmax + binary extraction
  6. Post-processing
  7. Save as .tif
...
def sliding_window_inference(
    ...
    Args:
        model: Trained UNet3DDeepSup (returns logits in eval mode).
        feat_extractor: GPU feature extractor module.
        volume: (Z, Y, X) uint8 raw CT volume.
    ...
    Returns:
        probs: (num_classes, Z, Y, X) probability volume.
    """
    model.eval()
    feat_extractor.eval()
    Z, Y, X = volume.shape
    ...
    print(f"  [V2] Sliding window: {total_patches} patches "
    ...
    with torch.no_grad():
        for batch_start in range(0, total_patches, batch_size):
            ...
            if done % 20 == 0 or done == total_patches:
                print(f"    [V2] Patch {done}/{total_patches}", end="\r")

    print(f"    [V2] Done: {total_patches} patches")

    count = np.maximum(count, 1.0)
    prob_avg = prob_sum / count[np.newaxis]
    prob_avg = prob_avg[:, :Z, :Y, :X]
    ...
    for fz in [False, True]:
        for fy in [False, True]:
            for fx in [False, True]:
                flip_configs.append((fz, fy, fx))

    for i, (fz, fy, fx) in enumerate(flip_configs):
        print(f"  [V2] TTA {i+1}/8: flip_z={fz}, flip_y={fy}, flip_x={fx}")
...
def run_inference(
    ...
    """Full inference pipeline for Pipeline V2."""
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    use_amp = device.type == "cuda"

    print(f"[V2] Device: {device}, AMP: {use_amp}")

    # Load model (V2: UNet3DDeepSup)
    ...
    print(f"  [V2] Predicted surface voxels: {pred_surface.sum()} "
          f"({pred_surface.sum() / pred_surface.size * 100:.1f}%)")

    if use_postprocess:
        print("[V2] Applying post-processing...")
        pred_surface = postprocess_prediction(pred_surface)
        print(f"  [V2] After post-processing: {pred_surface.sum()} voxels "
              f"({pred_surface.sum() / pred_surface.size * 100:.1f}%)")

    print(f"[V2] Saving prediction to {output_path}")
