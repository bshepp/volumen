...
echo "=== Vesuvius V3 training setup started at $(date) ==="
...
echo "=== Checking for existing V3 checkpoint on S3 ==="
RESUME_ARG=""
CKPT_PATH="${OUTPUT_DIR}/best_model_v3.pth"
if aws s3 cp "s3://${S3_BUCKET}/outputs_v3/best_model_v3.pth" "${CKPT_PATH}" --region us-east-1 2>/dev/null; then
...
echo "=== Starting Pipeline V3 (multi-scale fusion) training at $(date) ==="
chown -R ubuntu:ubuntu "${WORK_DIR}"
