...
This project has **two completely independent pipelines**. Read `PIPELINES.md` before making any changes.
...
1. **Never import across pipelines.** `src/` and `src_v2/` share zero code at the import level. File duplication is intentional.
2. **Every source file has a pipeline tag** on line 1: `# Pipeline: V1 (src/) — See PIPELINES.md` or `# Pipeline: V2 (src_v2/) — See PIPELINES.md`. Check this before editing.
3. **Checkpoints are not interchangeable.** V1 saves `best_model.pth`; V2 saves `best_model_v2.pth`. The architectures differ (V2 has auxiliary heads).
4. **V1 is frozen.** Do not modify files in `src/` unless explicitly asked to.

## Key Files

- `PIPELINES.md` — Full documentation of both pipelines, architecture diagrams, training commands, and warnings
- `requirements.txt` — Shared dependencies (V2 additionally requires `scikit-image`)
