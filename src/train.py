...
"""
Training loop for Vesuvius Challenge Surface Detection.

Key optimizations over Run 1:
  - GPU-accelerated feature computation (10-50x faster than scipy CPU)
  - Volume-grouped batching: each DataLoader item loads 1 volume, extracts
    multiple patches. Reduces disk I/O from O(batch_size) to O(1) per batch.
...
def train_one_epoch(
    ...
    """Train for one epoch."""
    model.train()
    running = {"total": 0.0, "ce_dice": 0.0, "cldice": 0.0, "boundary": 0.0}
    n_batches = 0

    optimizer.zero_grad(set_to_none=True)

    for batch_idx, (raw_images, labels) in enumerate(loader):
...
def main(args):
    ...
    print(f"=== Vesuvius Surface Detection Training (GPU Features v2) ===")
    ...
    feat_extractor.eval()
    print("GPU feature extractor initialized")
    ...
    print(f"Deprecated (excluded): {len(deprecated_ids)} samples")

    # Each dataset item loads 1 volume -> patches_per_volume patches
    ...
    print(f"Train: {len(train_dataset)} volumes -> {total_train_patches} patches/epoch")
    print(f"Val: {len(val_dataset)} volumes -> {total_val_patches} patches/epoch")

    # DataLoader: batch_size=1 means 1 volume -> patches_per_volume patches
    # collate_volume_batches flattens to (patches_per_volume, ...)
...
def get_args():
    parser = argparse.ArgumentParser(description="Vesuvius Surface Detection Training")
    parser.add_argument("--data-dir", type=str, required=True)
    parser.add_argument("--output-dir", type=str, default="outputs")
    parser.add_argument("--epochs", type=int, default=200)
    parser.add_argument("--batch-size", type=int, default=2, help="Not used directly; patches_per_volume controls effective batch size")
    parser.add_argument("--patch-size", type=int, default=128)
    parser.add_argument("--patches-per-volume", type=int, default=4, help="Patches extracted from each volume (effective batch size)")
