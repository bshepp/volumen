...
"""
Full inference pipeline for Vesuvius Challenge Surface Detection.
...
def sliding_window_inference(
    ...
    Args:
        model: Trained 3D U-Net (expects 6-channel input).
    ...
    print(f"  Sliding window: {total_patches} patches "
          f"({len(z_starts)}x{len(y_starts)}x{len(x_starts)}), "
          f"batch_size={batch_size}")

    # Normalize full volume once
    ...
    with torch.no_grad():
        for batch_start in range(0, total_patches, batch_size):
            ...
            # Build batch of raw patches: (B, 1, pz, py, px)
            ...
            if done % 20 == 0 or done == total_patches:
                print(f"    Patch {done}/{total_patches}", end="\r")
...
def test_time_augmentation(
    ...
    for i, (fz, fy, fx) in enumerate(flip_configs):
        print(f"  TTA {i+1}/8: flip_z={fz}, flip_y={fy}, flip_x={fx}")
...
def run_inference(
    ...
    # Run inference (features computed on GPU per-patch)
    if use_tta:
    ...
    print(f"  Predicted surface voxels: {pred_surface.sum()} "
